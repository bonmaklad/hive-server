...
async function deployRepo(site, token) {
  ...
  try {
    ...
    if (!fs.existsSync(path.join(buildDir, ".git"))) {
      ...
      // Always refresh origin with fresh installation token
      await run(`git remote set-url origin ${gitUrl}`, buildDir, deploymentId);
      await run(`git fetch origin`, buildDir, deploymentId);
    ...
    // Install dependencies (deterministic first, fallback if broken lockfile)
    try {
      await run(
        `bash -lc "set -a && source ${envPath} && npm ci --legacy-peer-deps --no-audit --no-fund"`,
        buildDir,
        deploymentId
      );
    } catch (e) {
      await logLine(deploymentId, "system", "npm ci failed — falling back to npm install");
      await run(
        `bash -lc "set -a && source ${envPath} && npm install --legacy-peer-deps --no-audit --no-fund"`,
        buildDir,
        deploymentId
      );
    }
    ...
    if (hasBuildScript) {
      await run(
        `bash -lc "set -a && source ${envPath} && npm run build"`,
        buildDir,
        deploymentId
      );
    } else {
      await logLine(
        deploymentId,
        "system",
        "No build script found; treating as node app (start only)."
      );
    }
