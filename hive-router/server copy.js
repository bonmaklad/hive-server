...
app.use(async (req, res, next) => {
  ...
  if (!siteId) {
    return res.status(404).send("Invalid dev URL");
  }
  ...
  // IMPORTANT:
  // Do NOT modify req.url
  // Framework handles basePath itself
  proxy.web(req, res, {
...
app.use(async (req, res) => {
  ...
  // Preview domain: <siteId>.hivehq.nz
  if (host.endsWith(".hivehq.nz")) {
    const siteId = host.replace(".hivehq.nz", "");
